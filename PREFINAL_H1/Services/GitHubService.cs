...
namespace PREFINAL_H1.Services
{
    public class GitHubService
    {
        ...
        public async Task<List<TaskModel>> GetShippedTasksAsync(string studentName, string targetDateStr, string owner, string repository)
        {
            ...
            string prUrl = $"https://api.github.com/repos/{owner}/{repository}/pulls?state=closed&per_page=100";
            ...
            {
                if (element.TryGetProperty("merged_at", out var mergedAtProp) && mergedAtProp.ValueKind != JsonValueKind.Null)
                {
                    string mergedAt = mergedAtProp.GetString() ?? "";
                    if (mergedAt.StartsWith(targetDateStr))
                    {
